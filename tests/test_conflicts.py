...
def test_merge_preserves_existing_first_detected():
    ...
    detected_today = [
        Conflict(
            ...
            bet_file="bet_z.md",
            owner="ceo",
            trigger="newly-rephrased",
            detector_note="updated",
            first_detected=date(2026, 4, 25),
        ),
        Conflict(
            id="C-bet_q-fresh",
    ...
    merged = merge_detected(existing, detected_today, modified_today=set())
...
def test_merge_drops_conflicts_for_modified_bets():
    ...
    merged = merge_detected(existing, detected_today=[], modified_today={"bet_z.md"})
    assert merged == []
